...
public class BurstBalloons {
    ...
    public int maxCoinsBottomUpDp(int[] nums) {
        ...
        Node N[][] = new Node[nums.length][nums.length];
        ...
        Node a = N[0][nums.length-1];
        ans.add(nums[a.index]);
        v.remove((Integer)a.index);
        for(int m=0;m<nums.length-1;m++){
            a = N[v.firstElement()][v.lastElement()];
            ans.add(nums[a.index]);
            v.remove((Integer)a.index);
        }
    ...
    public static void main(String args[]) {
        ...
        //System.out.println(bb.maxCoinsRec(input));
    }
}
